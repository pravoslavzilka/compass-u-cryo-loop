...
model TFCoilBusCoreLower
  ...
  // ===========================================================================
  // TF "core + lower limb" coil bus (single instance, all 112 channels).
  // Geometry: 112 parallel copper channels, 6x10mm elliptical cross-section,
  // hydraulic diameter 7.38mm, length 9.2m, total coil weight 13900kg
  // (ATEKO 22172-Z-R1 Tab.3, S5.2.3, PFD 22179-0-R0).
  // ===========================================================================

  parameter Modelica.Units.SI.Length length = 9.2 "Channel length.";
  ...
  parameter Modelica.Units.SI.Area outerCrossSectionalArea = 0.00155269
    "Outer cross-sectional area of one channel (metal + bore), sized so the total modeled copper mass equals 13900kg across all 112 channels.";
  parameter Modelica.Units.SI.CoefficientOfHeatTransfer alphaConstant = 1090
    "Gas-side convective heat transfer coefficient, Dittus-Boelter estimate at design flow.";
  parameter Modelica.Units.SI.ThermalResistance wallConductionR = 1.903e-6
    "Lumped wall conduction resistance for the whole component.";
  parameter Integer nChannels = 112 "Number of parallel channels.";
  ...
  parameter Integer assemblyIndex = 0 "Per-instance length offset index -- vestigial (single-instance branch), kept for symmetry with TFCoilBusUpper.";
  parameter Modelica.Units.SI.Thickness wallThickness = 0.018510
    "Equivalent wall thickness for 13900kg on a hydraulic-diameter basis -- reference only, not fed into tube2 (superseded by outerCrossSectionalArea).";
