...
model TFCoilBusCoreLower
  "One of the two 'core + lower limb' TF cooling busses (see TFCircuit.mo) --
  56 identical, lumped-parallel channels per bus. Geometry FROM SOURCE:
  ATEKO study 22172-Z-R1 Tab.3/S3.3.3 (TF core + lower limb: 112 channels
  total, equivalent diameter 7.379796 mm, length 9.2 m) -- this bus carries
  half that channel count (56), per the CALCULATED 4-bus split documented
  in docs/design-basis/tf-circulator-sizing.md S1. Modeled as
  nParallelTubes on a single Tube (PF's CoilAssembly.mo pattern), not as
  individually-branched channels (PF's CoilAssemblyNch pattern) -- correct
  here because, unlike PF's per-coil channels, every channel in one TF bus
  is geometrically identical, so there is nothing for separate branches to
  capture."
  outer ThermalSystems.SystemInformationManager sim;
  parameter Modelica.Units.SI.Length length = 9.2 "Channel length [m] -- FROM SOURCE, ATEKO 22172-Z-R1 Tab.3 (TF core + lower limb)";
  ...
  parameter Integer nChannels = 56 "Parallel channel count for this bus -- CALCULATED: 112 core+lower-limb channels (ATEKO Tab.3) / 2 busses, see tf-circulator-sizing.md S1";
  parameter Modelica.Units.SI.Power dischargeLoad = 0 "Peak discharge heat [W] -- 0, mirrors PFCircuit.mo's convention of zeroing dischargeLoads at circuit level (this model covers cooldown, not the tokamak-shot heat pulse itself)";
