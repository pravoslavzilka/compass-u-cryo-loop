...
model CSTest
  ...
  ThermalSystems.GasComponents.JunctionElements.VolumeJunction junction2(
    ...
    TInitial(displayUnit="K") = 80)
    annotation (Placement(transformation(extent={{-4,-4},{4,4}},
        rotation=90, origin={2,8})));
  CoilAssembly2ch cs1Assembly
    "Smoke-test the CS1/CS3U/CS3L (2-channel) coil-assembly class standalone -- mirrors PF/Test.mo's role for CoilAssembly, using the 2-channel variant since it's the one covered by isolation-valve control in CSCircuit.mo."
    annotation (Placement(transformation(extent={{28,34},{48,54}})));
equation
  connect(statePoint1.port, fan2ndOrder.portA) annotation (Line(
      points={{-28,-2},{-28,12}}, color={255,153,0}, thickness=0.5));
  connect(smoothStep.y, rotatoryBoundary.n_in)
    annotation (Line(points={{-55.4,20},{-56,19},{-48,19}}, color={0,0,127}));
  connect(rotatoryBoundary.rotatoryFlange, fan2ndOrder.rotatoryFlange)
    annotation (Line(points={{-44,19},{-44,20},{-36,20}}, color={135,135,135},
      thickness=0.5));
  ...
  connect(tube1.portB, junction2.portC) annotation (Line(
      points={{26,16},{2,16},{2,12}}, color={255,153,0}, thickness=0.5));
  connect(valve.portB, junction2.portA) annotation (Line(
      points={{2,-3},{2,4}}, color={255,153,0}, thickness=0.5));
  connect(junction2.portB, fan2ndOrder.portA) annotation (Line(
      points={{-2,8},{-28,8},{-28,12}}, color={255,153,0}, thickness=0.5));
  connect(cs1Assembly.portA1, fan2ndOrder.portB) annotation (Line(
      points={{25.2,44},{-28,44},{-28,28}}, color={255,153,0}, thickness=0.5));
  connect(cs1Assembly.portB1, tube1.portA) annotation (Line(
      points={{48.4,43.8},{48.4,16},{42,16}}, color={255,153,0}, thickness=0.5));
